...
import { Form, Input, Button, Tabs, Spin,Select } from 'antd';
import { useDispatch } from 'react-redux';
import { useGetStartupByIdQuery, useUpdateStartupMutation } from '../../../../../store/api/useStartup';
...
const EditStartup = () => {
  ...
  const { id } = useParams(); 
  const { data: startup, isLoading, refetch } = useGetStartupByIdQuery(id);
  const [updateStartup] = useUpdateStartupMutation();

  useEffect(() => {
    if (startup) {
      const { companyDetails, socialNetwork, legalDocs, logoDesign } = startup;

      form.setFieldsValue({
        ...companyDetails,
        ...socialNetwork,
        ...legalDocs,
        logoTypes: logoDesign.logos.map(logo => logo.type).join(', '),
      });
    }
  }, [startup, form]);
  ...
  const onFinish = async (values) => {
    ...
    try {
      await updateStartup({ id, updatedStartup: formattedValues }).unwrap();
      refetch(); // Rafraîchit les données après la mise à jour réussie
  ...
  if (isLoading) {
    return <Spin tip="Chargement des données..." />;
  }
